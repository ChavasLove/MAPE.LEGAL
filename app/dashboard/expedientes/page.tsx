    ...
    <div>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'rgba(94,107,123,0.3)' }}>
        <table className="w-full text-sm font-sans">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={9} className="px-4 py-10 text-center" style={{ color: '#A3A8AB', background: '#1F2A38' }}>Cargando...</td></tr>
            ) : visible.length === 0 ? (
              <tr><td colSpan={9} className="px-4 py-10 text-center" style={{ color: '#A3A8AB', background: '#1F2A38' }}>No hay expedientes con ese filtro.</td></tr>
            ) : visible.map(e => {
              const badge = ESTADO_BADGE[e.estado] ?? ESTADO_BADGE.activo;
              return (
                <tr key={e.id} style={{ borderTop: '1px solid rgba(94,107,123,0.2)', background: '#1F2A38' }}>
                  <td className="px-4 py-3 font-semibold text-white">{e.id}</td>
                  <td className="px-4 py-3" style={{ color: '#A3A8AB' }}>{e.cliente}</td>
                  <td className="px-4 py-3" style={{ color: '#A3A8AB' }}>{e.tipo}</td>
                  <td className="px-4 py-3" style={{ color: '#A3A8AB' }}>{e.municipio}</td>
                  ...
                  <td className="px-4 py-3 w-32">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 rounded-full" style={{ background: 'rgba(94,107,123,0.3)' }}>
                        <div className="h-1.5 rounded-full" style={{ width: `${e.legalidad}%`, background: '#2A8E50' }} />
                      </div>
                      <span className="text-xs" style={{ color: '#A3A8AB' }}>{e.legalidad}%</span>
                    </div>
                  </td>
