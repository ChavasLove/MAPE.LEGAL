...
- **Validación de env vars de auth**: `lib/authEnv.ts` (`checkAuthEnv()`) clasifica `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY` como `ok` / `missing` / `placeholder`. Trata whitespace-only y los placeholders del `.env.example` (`<project>`, `<your-`, `your-project`, `PASTE_`) como faltantes — esos pasaban el `if (!url)` truthy-check pero rompían Supabase. Ambos endpoints de login usan el helper, loggean la var específica vía `logAuthEnvFailure()` (visible en logs de Vercel) y devuelven `{ error: 'Configuración de servidor incompleta', code: 'SERVER_CONFIG' }`.
