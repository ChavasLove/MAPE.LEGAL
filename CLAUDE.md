...
- `GET /api/broadcast` — estado: último broadcast, suscriptores activos, precios más recientes. `requireRole('admin')` desde 2026-07-29 (hallazgo S4 del review externo — era cookie-only vía proxy)
...
- `GET /api/broadcast/config` — configuración de métricas del reporte diario. `requireRole('admin')` desde 2026-07-29
- `PATCH /api/broadcast/config` — cambiar métrica: `{ metric, action, currency?, patch?, updated_by? }`. `requireRole('admin')` desde 2026-07-29 — antes cualquier request con cookies inventadas podía mutar la config
- `GET /api/broadcast/prices?days=7` — historial de precios; `?latest=true` para solo el más reciente. `requireRole('admin'|'abogado'|'tecnico_ambiental')` desde 2026-07-29 (el precio público del día vive en `/api/precios/live`)
...
- **Runtime / diagnosticabilidad del webhook**: la ruta exporta `runtime='nodejs'`, `dynamic='force-dynamic'` y **`maxDuration=60`** — sin esto el default de Vercel mataba la invocación a mitad del pipeline (Supabase + embed OpenAI + 2 llamadas a Haiku) **antes** de llegar al outer catch, así que Twilio no recibía TwiML y el usuario veía **silencio total** (no el mensaje de error). El outer catch ahora **clasifica** el fallo en logs: `[maria] FATAL config error` (env `MARIA_CONFIG` — `getSupabase()` nombra la var faltante), `[maria] CLAUDE error` (`.status`/anthropic/overloaded), `[maria] DB error` (`PGRST*`/supabase), o `[maria] UNCLASSIFIED`. Cada request loggea un banner `[maria] handler start anthropic=… url=… svc=… openai=…` (solo presencia, nunca valores). Config errors devuelven el copy de mantenimiento; el resto el "problema técnico" — siempre **200 + TwiML**. Lookups opcionales usan `.maybeSingle()` (cliente, expediente subcmd, auto-register). **María-inbound es Twilio (TwiML en la respuesta HTTP, no necesita creds REST de Twilio); el broadcast es Meta Cloud API — fallan independiente.** **`maxDuration=60` NO se honra en Vercel Hobby (ceiling real ~10-15s) — por eso PR #184 acota cada await del path**: la llamada principal a Claude lleva `{ timeout: 10000, maxRetries: 0 }` (el default del SDK Anthropic es ~10min + 2 retries → un turno lento mataba la función antes del outer catch → silencio); `fetchAllPrices()` inline y `embedQuery` corren en `Promise.race` contra 4s; manual/concesiones/RAG corren en `Promise.all` (antes 3 round-trips secuenciales); la segunda llamada a Claude (extracción de datos del no-registrado) va envuelta en try/catch + bounded (su fallo ya no descarta la respuesta ya generada y guardada). Sin estos bounds el path secuencial excedía el deadline ~15s de Twilio y el usuario veía silencio — mismo síntoma que el 403 de firma.
...
- **Historial**: últimos 40 mensajes de `conversaciones_whatsapp` por número de WhatsApp (suficiente para sostener conversaciones multi-día sin truncar contexto importante). **Query es `order('created_at', {ascending:false}).limit(40)` + `.reverse()` en JS (PR #184)** — antes usaba `ascending:true` que traía los 40 más VIEJOS y anclaba la ventana ahí para siempre (memoria perdida + re-saludos en hilos >40 msgs, y pineaba cualquier fila mala al inicio de la ventana permanentemente). Antes de armar el `messages` de Claude: se dropean filas con `content` vacío y se hace shift de cualquier turn `assistant` inicial (`while (sanitizedHistory[0].role !== 'user') shift()`) — Anthropic exige que el primer mensaje sea `user`; sin esto un número que un admin contactó primero (take-over) tiraba 400 en cada turno. Mismo guard que el web widget.
...
- **Precios en tiempo real**: consulta `precios_diarios` del día; si no hay fila, llama `fetchAllPrices()` de `services/pricingService.ts` (goldapi.io spot con fallback a Yahoo Finance COMEX GC=F/SI=F + exchangerate-api.com). *(Nota: `services/metalsPriceService.ts` / `fetchLiveMetalPrices()` fue consolidado en `pricingService.ts` — ya no existe como archivo separado.)* El bloque `PRECIOS DE REFERENCIA` se inyecta en el system prompt con el precio internacional de referencia del oro (etiquetado "Oro internacional", **no "LBMA"** — la fuente real es goldapi.io spot / Yahoo COMEX GC=F, no el fixing LBMA), precio de compra MAPE LEGAL (80% del precio internacional en lempiras) y tipo de cambio BCH.
...
- **Widget** (`components/landing/MariaWidget.tsx`):
  ...
  - Welcome message **hard-coded** (lista de capacidades — bullets de cotizar / precios / pasos / concesiones / contacto; desde 2026-07-29 abre con "pequeña minería y minería artesanal", titulación antes que formalización, sin plata y con "referencia internacional" en vez de "LBMA"). **Display-only, filtrado del payload API** para que María no se enseñe a saludarse a sí misma. El filtro usa equality de contenido — si se edita `WELCOME_MESSAGE.content`, sesiones viejas en sessionStorage caen al shim defensivo del server (`while (validated[0].role !== 'user') validated.shift()`), no rompe pero pierde el guarantee.
...
- **La vista expone**: `numero_certificado`, `fecha_emision`, `peso_oro_g`, `estado`, `valido_hasta`, `hash_verificacion`, `mina_nombre`, `mina_codigo`, `mina_municipio`, `mina_departamento`. **Nunca PII del productor, monto de transacción, ni precio internacional de referencia.**
...
```bash
SEED_ADMIN_EMAIL=... SEED_ADMIN_PASSWORD=... node scripts/seed-super-admin.mjs
```
**Desde 2026-07-29 (hallazgo S2 del review externo) las credenciales vienen de env** — `SEED_ADMIN_EMAIL` + `SEED_ADMIN_PASSWORD` (≥12 chars), sin defaults. Las credenciales históricas estuvieron hardcodeadas en el repo y quedan en el historial de git: **si ese seed corrió alguna vez contra producción, rotar la contraseña de esa cuenta en Supabase Auth.**
...
TWILIO_VALIDATE_SIGNATURE      # (opcional, PR #176 → semántica invertida en PR #184) enforcement de X-Twilio-Signature en /api/whatsapp AHORA ES OPT-IN. 'true' = rechaza mismatches con 403; 'false' = salta la validación por completo (kill switch); unset/otro (DEFAULT) = valida + loguea mismatches pero DEJA PASAR el turno (nunca 403). El default enforce-on de PR #176 mandó María a silencio total (403 sin TwiML) cuando la URL reconstruida no calzaba — ver §Auditoría PR #184
TWILIO_SIGNATURE_LOG_ONLY      # (opcional) 'true' fuerza log-only aunque TWILIO_VALIDATE_SIGNATURE='true'. Redundante con el nuevo default (que ya es log-only); útil solo para forzar no-reject mientras enforce está flagueado
...
## Equipos — dossier técnico público + admin dormante (`/equipos`, `/admin/equipos`; pivote 2026-07-28)

No confundir con "Mercado de Proyectos" (`/admin/mercado`, documentos para junior ventures) — son features distintas.

> **PIVOTE 2026-07-28 — el marketplace de equipos fue descartado.** `/equipos` ya NO es e-commerce: es una **página técnica informativa estática** (server component prerendered `○`, cero Supabase, cero precios) sobre el tema *"Equipo para oro aluvial con alto contenido arcilloso"*. Contenido: hero técnico con 4 stats (100 TPH · ≥80% recuperación · 0–3 mm pulpa · 4 etapas), render del circuito + sección "la arcilla decide el diseño", **visor de láminas** (`app/equipos/SlidesViewer.tsx`, client: prev/next + miniaturas + flechas de teclado, sin autoplay) con las 7 láminas técnicas de `public/images/equipos/planta-lavado-oro-aluvial-100tph/02..08.jpeg` (01.jpeg = crop del render, usado como imagen del hero-section), 4 cards de etapas del proceso, tabla de equipos principales (header `--ink` per DESIGN §3), y cierre de **consulta técnica** (WhatsApp/correo — sin lenguaje de venta). Para cambiar el contenido: editar las constantes `SLIDES`/`ETAPAS`/`EQUIPOS_TABLA` en `app/equipos/page.tsx`; para reemplazar las láminas, sobrescribir los JPEG en `public/images/equipos/.../`.
>
> **Eliminado en el pivote:** `app/equipos/[slug]/` (detalle con precio), `EquiposShowcase.tsx`, y las APIs públicas `/api/equipos` + `/api/equipos/categorias` (exponían precios y ya nada las consumía). **Dormante pero funcional:** el stack admin completo — `/admin/equipos` (CRUD + importador de láminas con IA), `/api/admin/equipos*`, `equiposService`, tabla `equipos_mercado` (migraciones 027/029) y el registro real de la Planta 100 TPH en producción. Nada público lo renderiza; queda como backoffice de datos por si el marketplace revive. Si se decide eliminarlo del todo: borrar `app/admin/equipos`, `app/api/admin/equipos`, `services/equiposService.ts`, `lib/types/equipo.ts`, `scripts/seed-equipos.mjs`, el nav item `Equipos Mercado` de `app/admin/layout.tsx`, y las migraciones quedan históricas.
>
> Los detalles de vitrina/cap de abajo describen el **stack dormante** (admin), no la página pública.
>
> **Carga por láminas con IA (2026-07-28).** Flujo autoservicio en `/admin/equipos` → "Nuevo equipo" → panel "Importar desde láminas": (1) el admin selecciona JPEG/PNG/WebP (máx. 12 × 8 MB); (2) `POST /api/admin/equipos/upload-imagenes` (multipart, `requireRole admin|abogado`) las sube al **bucket público `equipos-imagenes`** (migración **029**, patrón 026: subida server-mediated con service-role, lectura por CDN público de Supabase); (3) `POST /api/admin/equipos/extraer` manda las URLs a **Claude Haiku 4.5 con vision** (lazy import del SDK dentro del handler — regla §Framework) que devuelve un borrador JSON (nombre, slug, descripciones, categoría validada contra allowlist, capacidad/potencia/peso/dimensiones, hasta 12 especificaciones); (4) el form queda pre-llenado, la primera imagen es portada (click en miniatura para cambiarla — la galería se arma como `uploadedUrls` menos portada), y el admin ingresa **el precio a mano — la extracción tiene prohibido devolver precios** (dato comercial, regla explícita en el prompt). El submit ahora siempre manda `galeria_urls` + `especificaciones` (en edición se siembran desde la fila → dejaron de ser inmutables desde el admin; las specs extraídas se pueden quitar una a una con ×). `isAllowedImageUrl` acepta además el host de Storage derivado de `NEXT_PUBLIC_SUPABASE_URL`, y `next.config.ts` agrega el remotePattern `*.supabase.co/storage/v1/object/public/**`.
>
> **Activación de la carga por láminas:** aplicar `029_equipos_storage.sql` en Supabase Studio (crea el bucket; idempotente). Sin ella, la subida devuelve el error accionable "¿está aplicada la migración 029?". La extracción requiere `ANTHROPIC_API_KEY` (ya presente para María).
...
- **Imágenes**: dos fuentes válidas — (a) hotlinked de `image.made-in-china.com` (host en `images.remotePatterns` de `next.config.ts`; sin esto `next/image` rechaza el host y la card crashea); (b) **first-party bajo `public/images/equipos/<slug>/`** referenciadas como ruta relativa `/images/equipos/...` — `isAllowedImageUrl` acepta cualquier path que empiece con `/images/` y `next/image` las sirve sin `remotePatterns`. Preferir (b) para productos propios (fotos/specs subidas por el equipo). Si el CDN del proveedor rompe hotlinking, migrar a Supabase Storage.
- **Primer producto real (2026-07-27)**: `planta-lavado-oro-aluvial-100tph` — Planta de Lavado de Oro Aluvial 100 TPH, **USD 118,000 CIF Honduras** (precio fijo, sin rango), `destacado: true, orden: 0` (encabeza el catálogo — el RPC ordena `destacado DESC, orden ASC`). 8 imágenes de specs en `public/images/equipos/planta-lavado-oro-aluvial-100tph/01..08.jpeg` (01 = portada — crop 4:3 del render, sin texto; 02–08 = galería de láminas). Circuito: tolva grizzly 25 m³ → alimentador vibratorio ~70 mm → trommel 1500 (70–100 TPH) → centrífugo STLB80 → 2× mesas 6S-4500 + 2× esclusas en colas → horno de fundición. **Los 12 productos demo fueron eliminados** (2026-07-27, `DELETE` en producción + removidos de `seed-equipos.mjs`) — el catálogo vive solo con productos reales; para agregar uno nuevo, añadir entrada al seed y re-correrlo (idempotente por slug).
...
- **Activación manual (Vercel NO la hace):** (1) aplicar `027_equipos_mercado.sql` en Supabase Studio → SQL Editor (idempotente; **aplicada en producción 2026-07-27**); (2) `node scripts/seed-equipos.mjs` (solo productos reales, idempotente por slug con `ignoreDuplicates`). Hasta entonces el catálogo renderiza vacío sin romper.
- **Deuda conocida:** Sin paginación en UI (la API la soporta vía limit/offset — el catálogo muestra los primeros 50). `galeria_urls`/`especificaciones` no son editables desde el form admin (sólo vía API/SQL).

## Widget de Precios en Vivo (`/precios`, `/api/precios/live`, `/admin/precios`, 2026-07-26)

Herramienta pública para el **minero artesanal** que muestra en una sola superficie: precio internacional del oro (USD/oz), **oro en Lempiras por onza troy** (int × TC), **precio de compra de oro de MAPE LEGAL en Lempiras/gramo** (80% del internacional), tipo de cambio USD/LPS, y el **precio del diésel en Honduras**. Audiencia distinta a la landing institucional (compradores/reguladores) — por eso vive en su propia página `/precios` (link compartible por WhatsApp/María), no embebida en `app/page.tsx`.

> **Solo oro — sin plata (2026-07-27, PR #190).** Se eliminó la plata de todas las superficies públicas del feature: el card "Plata LBMA" del widget `/precios`, la viñeta "Plata LBMA" del ticker sticky (`components/precios/PriceTickerBar.tsx`), y la fila "Plata internacional" del panel admin de metales. En cada caso el slot pasó a mostrar el **oro en Lempiras por onza troy** (`oro_lps_oz = oro_usd_oz × usd_hnl`, que la API ya computaba). La API `/api/precios/live` **sigue devolviendo** `plata_usd_oz` y `precios_diarios` sigue guardando `plata` (plumbing inofensivo, es lo que el fetch diario ya trae) — nada lo renderiza. María (WhatsApp + web) todavía puede citar plata si se lo preguntan (system prompt sin tocar).

> **Precio congelado a un snapshot diario de las 08:00 Honduras + timestamp de verificación (2026-07-27, PR #190).** Antes el precio podía cambiar a las **6 PM** Honduras (cuando rodaba el día UTC) porque el endpoint hacía un fetch nuevo en cada cache-miss. Ahora el precio se captura **una sola vez al día, a las 8 AM**, y queda fijo hasta la siguiente ventana de 8 AM. Ver los sub-bullets de "Anclaje a la ventana de 8 AM" y "Timestamp" abajo.

- **Metales + FX = feed en vivo** (reusa toda la infra de `precios_diarios` / `services/pricingService.ts` — GoldAPI → Yahoo COMEX → exchangerate-api, cache diario, mismo comportamiento de fin de semana documentado en §María). **Diésel = precio regulado** que la Secretaría de Energía (SEN) fija **semanalmente** (lunes 6 AM, con subsidio del 50% al incremento internacional) — NO hay API JSON confiable, así que se modela como valor almacenado + `vigente_desde`, editable por admin. La página lo etiqueta con la fecha de vigencia y la fuente (SEN); no se pretende un tick por segundo.
- **Anclaje a la ventana de 8 AM** (`services/pricingService.ts:effectivePriceDate(now?)`, `PRICE_REFRESH_HOUR_HN = 8`): devuelve la fecha Honduras (`YYYY-MM-DD`) de la ventana de las 08:00 a la que pertenece el instante (hoy si `hora ≥ 8` HN, ayer si `< 8`). Es **la única llave de caché de precios**. `storePrices(precios, fecha = effectivePriceDate())` llavea `precios_diarios.fecha` por esa ventana (antes era `toISOString().slice(0,10)` = fecha UTC, rollover a las 6 PM HN). **Todos los lectores por fecha fija están alineados a esta llave** — `/api/precios/live`, `app/api/maria/chat` (`buildPriceContext`) y el webhook `app/api/whatsapp/route.js` — para que un cache-miss vespertino no re-fetchee y sobrescriba la fila congelada. Los lectores "último registro" (`getLatestPrices`, `/api/admin/maria/stats`) no usan fecha fija y no se ven afectados. El cron de broadcast (14:00 UTC = 8 AM HN) captura el snapshot vía `fetchAndStorePrices()`; si el cron cae, el endpoint `/api/precios/live` lo captura en la primera visita después de las 8 AM.
- **Migración 028** (`028_precios_combustibles.sql`) — tabla `precios_combustibles` (unique `combustible` con CHECK de 5 valores: `diesel|gasolina_regular|gasolina_superior|kerosene|glp`, `precio_hnl numeric(10,2) > 0`, `unidad`, `zona`, `vigente_desde`, `fuente`, `activo`, soft-delete). RLS mismo patrón que 027: lectura pública `activo=true`, escritura `admin|abogado|tecnico_ambiental` vía `user_roles`, `FOR ALL TO service_role` explícita (el service_role del proyecto no tiene BYPASSRLS). Seed idempotente **`ON CONFLICT DO NOTHING`** (el admin es dueño de los valores tras el primer apply — un re-run NUNCA clobbea una edición). Sin RPC — writes van por service-role client + policy (igual que equipos/027). Diésel sembrado: L124.26/gal Tegucigalpa vigente 2026-07-27.
- **Tipos**: `lib/types/combustible.ts` — `TipoCombustible`, `COMBUSTIBLE_LABELS`, `TIPOS_COMBUSTIBLE`, `isTipoCombustible`, `Combustible`. **`COMBUSTIBLE_LABELS`/`TIPOS_COMBUSTIBLE` deben mantenerse en sync con el CHECK de la migración 028.**
- **Servicio**: `services/combustiblesService.ts` — reads públicos vía proxy anon `supabase` (`getCombustibles`, `getCombustible`); reads/writes admin vía `getAdminClient()` **dentro de cada función** (`listCombustiblesAdmin`, `upsertCombustible` con `onConflict:'combustible'`). `normalize()` coacciona `precio_hnl` a `Number` (PostgREST retorna numeric como string). Reads no-fatales: degradan a `[]` si la migración 028 no está aplicada.
- **API pública** `GET /api/precios/live` — sin auth, **rate-limited 60/5min por IP** (`lib/rateLimit.ts`), `Cache-Control: public, s-maxage=60, stale-while-revalidate=300`. Lee `precios_diarios` de la **ventana de 8 AM** (`effectivePriceDate()`) vía service-role (anon no tiene SELECT en esa tabla, migración 009). **Si la fila de la ventana existe la sirve tal cual y NUNCA re-fetchea** — así el precio queda congelado de 8 AM a 8 AM. Solo cuando la fila falta (cron no corrió / primer visitante después de las 8 AM) hace **un** `fetchAllPrices()` con `Promise.race` contra timeout duro **8s** + `storePrices(live, priceDate)` write-back llaveado a la misma ventana. Lee combustibles vía `getCombustibles()`. Computa el precio de compra con `mapeGoldBuyLpsPerGram()`. Devuelve `{ oro_usd_oz, usd_hnl, oro_lps_oz, oro_compra_lps_gramo, mape_factor, diesel, combustibles, consultado_hn, actualizado_hn, fecha, fetched_at, fuente, plata_usd_oz }` (`plata_usd_oz` sigue presente pero no se renderiza).
- **Timestamp de verificación** (`actualizado_hn`): la API formatea `precios_diarios.fetched_at` (la hora real en que se capturó el precio del upstream) en hora Honduras (`America/Tegucigalpa`, `hour12`). Es el timestamp que el minero/comprador puede confiar — es la captura de las 8 AM, **no** la hora de consulta (`consultado_hn`, que sigue disponible como la hora de la request). El widget lo muestra como "Precio capturado: …"; el panel admin como "· Capturado …".
- **Helper anti-drift** (`services/pricingService.ts`): `mapeGoldBuyLpsPerGram(oroUsdOz, usdHnl) = oro*0.8*fx/TROY_OUNCE_GRAMS` + `MAPE_GOLD_BUY_FACTOR = 0.8`. **Fuente única** del precio de compra — `broadcastService.generateDailyMessage` y `app/api/maria/chat` fueron refactorizados a este helper (behavior-preserving) para que boletín, María (WhatsApp + web) y el widget nunca driftéen.
- **Widget** `components/precios/LivePricesWidget.tsx` (`'use client'`, prop `lang`): fetchea `/api/precios/live` on-mount, **poll cada 60s con guard `document.hidden`** + refetch on `visibilitychange`, `AbortController` con timeout 12s + cleanup on unmount (el poll solo re-lee la fila congelada, no dispara upstream). Card primaria destacada (moss-tint) = compra oro L/gramo; grid secundario = **oro int (USD/oz), oro en Lempiras (L/oz troy), USD/LPS, diésel** (con `vigente_desde`). Pie: "Precio capturado: {`actualizado_hn`} (Honduras) · Fuente: …" + nota "El precio del oro se actualiza una vez al día, a las 8:00 a.m. (Honduras), y se mantiene fijo durante el día para su verificación." Dot "En vivo" **estático** (rojo si offline; sin `animate-pulse` per DESIGN §4). Skeleton estático. Mantiene el snapshot viejo en pantalla si un poll falla (solo el dot se pone rojo). Cero hex; numerales `--earth`/`--font-display`.
- **Ticker sticky** `components/precios/PriceTickerBar.tsx` (`'use client'`, prop `lang`, `TICKER_HEIGHT = 36`): barra delgada arriba del landing (`app/page.tsx`). Viñetas mono scrollables: Compra oro (L/g, acento sand), Oro LBMA (USD/oz), **Oro Lps (L/oz troy)**, USD, Diésel + link "Ver precios →" a `/precios`. Mismo `/api/precios/live`, poll 60s con guard `document.hidden`. Sin timestamp inline (el detalle vive en `/precios`).
- **Página pública** `app/precios/page.tsx` (server, metadata + OG) → `PreciosClient.tsx` (`'use client'`, nav propia, toggle ES/EN vía `ml_lang`, CTA WhatsApp miner-facing `wa.me/50497373139`, `<MariaWidget>`). Link `/precios` agregado a `NAV_LINKS` de `app/page.tsx`.
- **Admin** `/admin/precios` (gateado por layout admin) — `page.tsx` (server, fetch `listCombustiblesAdmin` + `getLatestPrices`, degrada pre-migración) → `AdminPreciosClient.tsx`: editor por combustible (precio, unidad, vigente_desde, zona, fuente, visible-al-público) con `busy` lock por fila + status `role=status/alert`; panel read-only de metales en vivo (oro int, **oro en Lempiras L/oz troy** — sin plata) con "Registro del {fecha} · Capturado {`fetched_at` en HN} · Fuente" + nota "El precio del oro se captura una vez al día, a las 8:00 a.m. (Honduras)". API `GET/PATCH /api/admin/precios/combustibles` (`requireRole`: GET admin/abogado/tecnico, PATCH admin/abogado; valida allowlist + precio>0 + formato fecha + caps de string; `updated_by = auth.user.email`; no filtra `error.message`). Nav item "Precios / Diésel" (ícono `Fuel`) en `app/admin/layout.tsx`.
- **proxy.ts NO cambió**: `/precios` y `/api/precios/*` no están en el matcher (públicos por default); `/admin/precios` y `/api/admin/precios` ya cubiertos por `/admin/:path*` + `/api/admin/:path*`.
- **Activación manual (Vercel NO la hace):** aplicar `028_precios_combustibles.sql` en Supabase Studio → SQL Editor (idempotente; incluye el seed). Hasta entonces el widget muestra métricas de metales pero oculta el card de diésel; ni la página pública ni el admin rompen.
- **Runbook — actualizar diésel cada lunes:** `/admin/precios` → editar el precio del diésel (y gasolinas/kerosene) con la nueva estructura SEN + `vigente_desde` del lunes → Guardar. El widget refleja el cambio en ≤60s (TTL de cache). La fecha `vigente_desde` visible en el widget hace que un valor viejo sea evidente, no silencioso.
- **Deuda / follow-up:** María no cita el precio del diésel todavía (solo oro/FX) — inyectar `diesel` en `buildPriceContext` de `app/api/maria/chat` + el system prompt es el siguiente paso natural para el minero que pregunta por WhatsApp. La plata sigue viva en el plumbing (`plata_usd_oz` en la API, `precios_diarios.plata`, `buildPriceContext` de María) aunque ninguna superficie pública la renderiza — si se quiere quitar del todo hay que tocar el system prompt de María y limpiar el plumbing. Sin scrape en vivo de sen.hn (evaluado y descartado por fragilidad; el modelo stored+admin es la fuente confiable). `zona` es un solo valor (Tegucigalpa) — San Pedro Sula tiene precios ligeramente distintos por distribución; agregar por-zona requiere cambiar el unique key a `(combustible, zona)`.
...
- **Firma de webhooks** — `lib/webhookSignatures.ts` (validadores HMAC timing-safe, sin dependencia nueva): Twilio `X-Twilio-Signature` (SHA1 sobre URL+params ordenados) en `/api/whatsapp`, Meta `X-Hub-Signature-256` (SHA256 sobre raw body) en `/api/webhook/whatsapp`. **⚠️ El default enforce-on de este PR causó un outage de María WhatsApp — corregido en PR #184 (ver §Auditoría abajo).** El rollout original (`TWILIO_SIGNATURE_LOG_ONLY='true'` primero) asumía que el operador seteara el flag; sin él, enforce-on + URL reconstruida que no calzaba (dominio custom vs `*.vercel.app`, www vs apex, Messaging Service callback) → 403 sin TwiML → silencio total. Meta: se omite (logueado) hasta setear `WHATSAPP_APP_SECRET`. También hizo no-fatal (`.catch`) el primer insert a `transacciones_pendientes` (antes un blip de Supabase reemplazaba la respuesta de María con "problema técnico").
...
### María WhatsApp (Twilio) no respondía — outage por firma + timeouts (PR #184, 2026-07-26)

**Síntoma reportado:** "María funciona en el web page pero en WhatsApp no responde." El número está en **Twilio** (confirmado por el operador). El web widget (`/api/maria/chat`) funcionaba → Anthropic / Supabase / OpenAI / RAG / system prompt compartidos están sanos → el fallo era **específico del transporte WhatsApp** (`/api/whatsapp`, ruta Twilio, TwiML).

**Diferencial clave:** la ruta web y la ruta WhatsApp comparten el cerebro pero la WhatsApp tiene dos cosas que la web no: (a) el chequeo de firma Twilio, (b) más latencia (2 llamadas a Claude + fetch de precios inline, todo secuencial y sin bound). Ambas rompen **solo** el canal WhatsApp — consistente con "web sí, WhatsApp no".

**Causa raíz #1 — firma Twilio fail-closed por default (de PR #176).** El gate `if (twilioToken && TWILIO_VALIDATE_SIGNATURE !== 'false')` enforce-aba apenas `TWILIO_AUTH_TOKEN` estuviera set (es var requerida para el contact-forwarding a Willis). Validaba contra **una sola URL adivinada** desde `NEXT_PUBLIC_SITE_URL`; cualquier mismatch con la URL que Twilio firmó (dominio custom vs `*.vercel.app`, www vs apex, Messaging Service callback, rewrites del proxy) → `403 "Firma inválida"` **sin TwiML** → el usuario ve silencio total. Fail-closed sobre un valor que solo se puede adivinar = black-hole del webhook entero.

**Causa raíz #2 — awaits sin bound exceden el deadline ~15s de Twilio.** `maxDuration=60` NO se honra en Vercel Hobby (ceiling real ~10-15s). La llamada principal a Claude no tenía timeout (default del SDK Anthropic ~10min + 2 retries); `fetchAllPrices()` corría inline hasta ~16s en cache fría; `embedQuery` hasta ~15s (5s×3). Corriendo secuencial, la función se mataba antes del outer catch → sin TwiML → silencio (mismo síntoma que el 403).

**Bugs adicionales de correctness (mismo PR, hallados por auditoría de 3 agentes paralelos):**
- **Historial traía los 40 mensajes más VIEJOS** (`ascending:true` + `limit(40)`), no los últimos — memoria perdida + re-saludos en hilos >40 msgs, y anclaje permanente de cualquier fila mala al inicio de la ventana. Fix: `ascending:false` + `.reverse()`.
- **Faltaba el guard de `assistant` inicial** (el web widget lo tiene): un número que un admin contactó primero (take-over) tiraba Anthropic 400 en cada turno → "problema técnico" permanente. Fix: `while (sanitizedHistory[0].role !== 'user') shift()` + drop de filas vacías + guard de `content` null.
- **La 2da llamada a Claude (extracción) estaba sin envolver** en el response path → su fallo descartaba la respuesta ya generada y guardada, devolviendo un falso "problema técnico". Fix: try/catch + bounded.
- Reply vacío de Claude ya no se persiste (rompía el turno siguiente); `interpretAndExecute` envuelto; `esc()` strippea control chars ilegales de XML antes de armar TwiML.

**Fix de firma (PR #184):** `twilioCandidateUrls()` valida contra **todas** las URLs plausibles (site URL, forwarded-host, request.url) y acepta si **alguna** calza; y enforcement es **opt-in** — `403` solo con `TWILIO_VALIDATE_SIGNATURE==='true'`; el default valida + loguea mismatches pero deja pasar el turno. Un misconfig de URL/token ya no puede tumbar María.

**Unblock inmediato sin deploy** (mientras PR #184 no esté mergeado): setear `TWILIO_VALIDATE_SIGNATURE='false'` en Vercel → Env Vars (Production) + redeploy — el código viejo honra ese kill switch y salta el bloque de firma.

**Diagnóstico decisivo** (una línea de log al mandar un texto de WhatsApp): `[maria] handler start …` presente + `[maria] twilio signature mismatch` → era la firma; `handler start` presente y luego silencio/timeout → latencia; **ningún** `handler start` → Twilio no está pegándole al endpoint (config de consola/número/sandbox, fuera del código). Nota de arquitectura: existen DOS webhooks — `/api/whatsapp` (Twilio, texto) y `/api/webhook/whatsapp` (Meta, solo media; **descarta todo texto** en `route.ts:54`). Un número está en UN solo proveedor; con el número en Twilio, la ruta Meta es dead code para el inbound de texto.

**Lección recurrente:** un chequeo de autenticidad fail-closed sobre un valor no-determinístico (URL detrás de proxy) debe fail-OPEN-con-log por default, no enforce-on. Y todo await en el response path de un webhook con deadline duro (Twilio ~15s) debe estar bounded — el default del SDK Anthropic (~10min) es una bomba de latencia. El web widget ya tenía ambos patrones (parallel context, guard de rol, bounds); la ruta WhatsApp se quedó atrás.
