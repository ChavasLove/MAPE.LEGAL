...
Herramienta pública para el **minero artesanal**. Dos superficies alimentadas por el mismo endpoint `/api/precios/live`: (1) un **ticker sticky minimalista** arriba de todo en la landing (`components/precios/PriceTickerBar.tsx`, lo primero que se ve), y (2) una **página dedicada `/precios`** (link compartible por WhatsApp/María) con el widget completo `LivePricesWidget`. **Set canónico de exactamente 5 métricas** (a pedido del owner, 2026-07-27): **Oro LBMA** (USD/oz), **Plata LBMA** (USD/oz), **tasa USD→LPS**, **precio de compra de oro de MAPE LEGAL en Lempiras/gramo — solo el monto, sin mostrar el porcentaje (80%)**, y **precio del diésel en Honduras** (LPS/galón). Nada más se muestra en las superficies públicas (las gasolinas/kerosene se administran pero no se despliegan).

> **Nota "LBMA":** las etiquetas de oro/plata dicen **"LBMA"** por decisión del owner (es el benchmark que compradores/mineros reconocen), pero la **fuente real es goldapi.io spot / Yahoo COMEX GC=F/SI=F**, NO el fixing oficial de la LBMA. Por eso el resto del código (María WhatsApp + web, boletín diario) sigue etiquetando **"Oro internacional"** — ese label NO se cambió. Si algún día se unifica a "LBMA", tocar `lib/maria/systemPrompt.ts` §8 + `broadcastService` + `app/api/maria/chat`.
...
- **API pública** `GET /api/precios/live` — sin auth, **rate-limited 60/5min por IP** (`lib/rateLimit.ts`), `Cache-Control: public, s-maxage=60, stale-while-revalidate=300`. Lee `precios_diarios` de hoy vía service-role (anon no tiene SELECT en esa tabla, migración 009); si falta o `oro` es null, hace `fetchAllPrices()` con `Promise.race` contra timeout duro **8s** + write-back de cache (mismo patrón que `app/api/maria/chat`). Lee combustibles vía `getCombustibles()`. Computa el precio de compra con `mapeGoldBuyLpsPerGram()`. Devuelve `{ oro_usd_oz, plata_usd_oz, usd_hnl, oro_lps_oz, oro_compra_lps_gramo, mape_factor, diesel, combustibles, consultado_hn, fecha, fuente }`. La key de fecha es UTC (`toISOString().slice(0,10)`) para calzar con `storePrices()`.
...
- **Ticker de la home** `components/precios/PriceTickerBar.tsx` (`'use client'`, prop `lang`, export `TICKER_HEIGHT = 36`): barra delgada `sticky top:0 z-index:101` sobre fondo `--ink`, **primer elemento de `app/page.tsx`** (antes de `<nav>`). Una sola línea mono, scroll horizontal en móvil (clase `.no-scrollbar` en `globals.css`), dot "En vivo" **estático**. 5 métricas: Compra oro (acento `--sand`, solo monto), Oro LBMA, Plata LBMA, USD, Diésel + link `/precios`. Mismo fetch/poll 60s (`document.hidden` guard) + `AbortController` que el widget. **La `.nav` (y `.nav-mobile-panel`) reciben `style={{ top: TICKER_HEIGHT }}` / `top: TICKER_HEIGHT + 64` inline en `app/page.tsx`** para apilar sticky debajo del ticker sin editar la posición base en `globals.css`.
- **Widget** `components/precios/LivePricesWidget.tsx` (`'use client'`, prop `lang`): fetchea `/api/precios/live` on-mount, **poll cada 60s con guard `document.hidden`** + refetch on `visibilitychange`, `AbortController` con timeout 12s + cleanup on unmount. Card primaria destacada (moss-tint) = compra oro L/gramo (**solo monto, sin `%`** — el prop `showBuyFactor` se removió; el `mape_factor` del API ya no se muestra); grid secundario = **Oro LBMA, Plata LBMA, USD/LPS, diésel** (con `vigente_desde`). **NO** muestra "otros combustibles" ni el `≈ L/onza` derivado (removidos en el trim 2026-07-27). Dot "En vivo" **estático** (rojo si offline; sin `animate-pulse` per DESIGN §4). Skeleton estático. Mantiene el snapshot viejo en pantalla si un poll falla (solo el dot se pone rojo). Cero hex; numerales `--earth`/`--font-display`.
- **Página pública** `app/precios/page.tsx` (server, metadata + OG) → `PreciosClient.tsx` (`'use client'`, nav propia, toggle ES/EN vía `ml_lang`, CTA WhatsApp miner-facing `wa.me/50497373139`, `<MariaWidget>`). Link `/precios` agregado a `NAV_LINKS` de `app/page.tsx`. La home **NO** tiene una sección grande de precios — solo el ticker (una sección `#precios-hoy` con `LivePricesWidget` se probó y se removió por redundante con el ticker).
- **Admin** `/admin/precios` (gateado por layout admin) — `page.tsx` (server, fetch `listCombustiblesAdmin` + `getLatestPrices`, degrada pre-migración) → `AdminPreciosClient.tsx`: editor por combustible (precio, unidad, vigente_desde, zona, fuente, visible-al-público) con `busy` lock por fila + status `role=status/alert`; panel read-only de metales en vivo. API `GET/PATCH /api/admin/precios/combustibles` (`requireRole`: GET admin/abogado/tecnico, PATCH admin/abogado; valida allowlist + precio>0 + formato fecha + caps de string; `updated_by = auth.user.email`; no filtra `error.message`). Nav item "Precios / Diésel" (ícono `Fuel`) en `app/admin/layout.tsx`.
