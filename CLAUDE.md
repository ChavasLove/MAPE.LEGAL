...
- **Servicio**: `services/equiposService.ts` — reads públicos vía proxy anon `supabase` + RPCs; writes admin vía `getAdminClient()` **dentro de cada función** (nunca module-level). `deleteEquipo` es soft-delete (`activo = false`) — el catálogo público nunca ve inactivos.
- **Rutas API**: `GET /api/equipos` (público, valida categoria contra allowlist, cache 60s + SWR 5min) · `GET /api/equipos/categorias` (público, cache 300s) · `GET/POST /api/admin/equipos` + `PATCH/DELETE /api/admin/equipos/[id]` (`requireRole`: GET admin/abogado/tecnico_ambiental, writes admin/abogado; PATCH con whitelist de campos; 23505 → 409; slug regex `^[a-z0-9-]+$`).
