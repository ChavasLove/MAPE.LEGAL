...
### Viewport meta móvil — resuelto 2026-05-14 (PR #137, commit `116a93d`)
- **Síntoma**: en iPhone reales la landing se veía con 3 columnas estrechas en Contacto ("WHATSAPP INSTITUCIONA" / "gerencia@ma" truncados), 2 columnas apretadas en Verificación ("PESO CERTIFI" / "VÁLI HAST" truncados) y la `MapLegend` permanecía expandida cubriendo el mapa. Síntomas reportados por el usuario con screenshots de iPhone, pero NO reproducibles en DevTools mobile-emulator porque éste fuerza viewport correcto.
- **Causa raíz**: `app/layout.tsx` no exportaba `viewport`. Sin él, mobile Safari / Chrome Android renderizan a ~980-1024 px CSS y escalan visualmente — por eso `lg:grid-cols-3` (contact) y `lg:grid-cols-2` (verificación) sí disparaban, y la `matchMedia('(max-width: 639px)')` de `MapLegend` nunca matcheaba. Todo el trabajo responsive de PR #126 quedaba neutralizado por este single missing export.
- **Fix**: agregar `export const viewport: Viewport = { width: 'device-width', initialScale: 1, viewportFit: 'cover', themeColor: '#1F2A38' }` en `app/layout.tsx`. Tras esto las clases Tailwind responsive, `.mape-section`, hamburger nav, y `MapLegend` auto-collapse hacen su trabajo. Cero cambios adicionales necesarios.
- **Lección operativa**: el quality gate de PR #126 declaró el plan "responsive complete" sin probar en device real porque DevTools emulator pasaba — pero el emulador inyecta su propio viewport para forzar render correcto. **Cualquier futura validación móvil debe incluir un device físico o `responsive-design-mode` real, no sólo Chrome DevTools.** Próxima vez que se duda de un fix móvil, primero verificar `<meta name="viewport">` en el DOM rendered antes de buscar bugs en CSS.

### Brand scope nacional — landing + María (PR #137, commit `8037b38`, 2026-05-14)
- **Cambio**: despinear superficies públicas del piloto Iriona, Colón y de la relación comercial con Chiopa Industrias (downstream buyer). La página pasa de "operamos en Iriona" a "trabajamos con asociaciones de mineros en Honduras", y la frase "intermediario… con la refinería Chiopa Industrias" se reescribe a "intermediario… con la cadena formal de comercialización de oro responsable". Decisión de negocio: CHT opera nacional, Chiopa queda como relación privada hasta nuevo aviso.
- **Surfaces tocadas**: `app/page.tsx` (dos párrafos de identidad + campo `Unidad minera` del mockup → "Distrito minero — Honduras"), `app/api/whatsapp/route.js` (system prompt de María — quita "especialmente en Iriona, Col.", "COMPRA DE ORO (via Chiopa Industrias)" → "COMPRA DE ORO", "Alcaldía de Iriona" → "Alcaldía municipal correspondiente"; reporte ejecutivo admin coherente), `components/decor/TopoBand.tsx` (label SVG `85°08′W · IRIONA, COLÓN` → `85°00′W · HONDURAS`), `app/login/page.tsx` (decor `IRIONA, COLÓN, HONDURAS` → `HONDURAS · CENTROAMÉRICA`).
- **Cambio de comportamiento backend**: los 3 fallbacks `municipio: 'Iriona, Colón'` en `route.js` (uno en auto-register, dos en la rama de extracción JSON post-respuesta) ahora **omiten el campo del INSERT** cuando el extractor no devuelve municipio — la columna `clientes.municipio` es nullable (verificado en `008_piloto_core.sql:16`). Los clientes nuevos registrados vía María a partir de ahora tienen `municipio = NULL` cuando no lo proveen, en vez de quedar sesgados a Iriona. El display al system prompt usa `cliente.municipio || 'no registrado'` (alineado con `situacion_tierra || 'no registrada'` de la línea vecina). Migraciones SQL aplicadas (`004`, `006`, `008`, `021`) con seed pinned a Iriona quedan intactas — los registros viejos en BD no se tocan.
- **Out of scope explícito**: `README.md` (sigue mencionando "Piloto Iriona 2026" y Chiopa en 4 líneas — doc interna del repo privado), `public/dashboard.html` (HTML estático legacy con un solo placeholder), `services/onboardingService.ts:191` (default municipal en flow `startOnboarding()` que no se invoca en práctica — tabla `onboarding_states` de migración 010 no aplicada).
- **Cómo verificar tras deploy**: (a) abrir landing, buscar "Iriona"/"Chiopa" en DevTools — debe dar 0 hits; (b) preguntarle a María "¿en dónde operan?" / "¿a quién venden el oro?" — la respuesta NO debe pinear a Iriona ni nombrar a Chiopa; (c) cliente nuevo sin municipio se registra vía María → fila en `clientes` con `municipio = NULL`. La grep sweep canónica:
  ```bash
  grep -rni "iriona\|chiopa" app/page.tsx app/login/page.tsx components/decor/ app/api/whatsapp/route.js
  ```
