...
## Mercado de Equipos — vitrina de maquinaria (`/equipos`, `/admin/equipos`, 2026-07-11; rediseño vitrina 2026-07-28)

Vitrina pública de equipos de lavado de oro para mineros artesanales. No confundir con "Mercado de Proyectos" (`/admin/mercado`, documentos para junior ventures) — son features distintas.

> **Rediseño 2026-07-28 — vitrina, no catálogo masivo.** El inventario real nunca supera **3 equipos activos** (`MAX_EQUIPOS_ACTIVOS = 3` en `lib/types/equipo.ts`, enforced server-side: POST crea → 409 si ya hay 3 activos; PATCH `activo: true` → 409 contando con `countEquiposActivos(excludeId)`). La página pública `/equipos` dejó de ser un catálogo con buscador/filtros/sidebar y pasó a ser **vitrina editorial**: `app/equipos/EquiposShowcase.tsx` (server component, cero estado) renderiza hasta 3 equipos como feature cards grandes alternadas (portada 4:3 + categoría + nombre + 4 specs clave + precio `--earth`/Playfair + Incoterm de `especificaciones.Incoterm` + CTAs ficha/WhatsApp), más un cierre "equipo bajo pedido". `EquiposCatalogClient.tsx` fue **eliminado** (y con él los searchParams de filtros — `page.tsx` ya no los lee; el RPC `search_equipos_mercado` sigue vivo para `/api/equipos`). Data path de la vitrina: `getEquiposVitrina(3)` en `equiposService` — select directo con anon proxy (la RLS de 027 sólo expone `activo=true`), orden `destacado DESC, orden ASC, precio ASC`, degrada a vacío si 027 no está aplicada.
>
