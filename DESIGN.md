...
- **Las utilidades de Tailwind que deban usar una fuente de marca tienen que estar mapeadas en `@theme inline`.** Hoy hay una: `--font-sans: var(--font-body)`. Sin ese mapeo, `--font-sans` conserva el default del framework (`ui-sans-serif, system-ui, …`) y la clase `font-sans` **sustituye Inter por la fuente del sistema operativo**. Estuvo así en 190 puntos hasta 2026-08-05. Al añadir una utilidad de tema, verificarla en el CSS **compilado**, no en el fuente. `--font-mono` no va en `@theme` (el `:root` ya lo redefine después del de Tailwind); `--font-serif` tampoco (sin usos).
- **Iconos de marca:** `app/favicon.ico` (16/32/48/64), `app/icon.png` (512) y `app/apple-icon.png` (180) — convenciones de archivo del App Router, sin nada en `layout.tsx`. Los tres llevan el **lockup completo idéntico en toda resolución**, derivados de un maestro único. No recortar solo la marca, no variar el encuadre por archivo y **no aplicar realce de saturación a los tamaños chicos** (produce un bloque amarillo a 16 px). Los frames del `.ico` van en **RGBA** o el build de Turbopack falla. Detalle en `CLAUDE.md` §Iconos de marca.
- **Stack snippets** (CSS, Tailwind config, Style Dictionary JSON): ver README §Color Manual.
- **Future work:** cualquier color hardcodeado fuera de `globals.css` es deuda técnica. Si encuentras uno, abre PR para reemplazarlo por el token correspondiente. Inventario medido el 2026-08-05: **565 ocurrencias**, concentradas en `app/dashboard/**` y `app/portal/**`; seis valores no existen en la paleta. Ver `docs/auditoria-marca-2026-08.md` §5.
